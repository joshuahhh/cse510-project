...
// JAH: not exported by @codemirror/gutter, boo
...
const activeGuttersRight = Facet.define<Required<GutterConfig>>()

/// Define an editor gutter. The order in which the gutters appear is
/// determined by their extension priority.
export function gutterRight(config: GutterConfig): Extension {
  return [guttersRight(), activeGuttersRight.of({...defaults, ...config})]
}
...
const baseTheme = EditorView.baseTheme({
  ...
  "&light .cm-gutters-right": {
    backgroundColor: "#f5f5f5",
    color: "#6c6c6c",
    // borderLeft: "1px solid #ddd"
  },

  "&dark .cm-gutters-right": {
...
export function guttersRight(config?: {fixed?: boolean}): Extension {
  let result = [
    gutterRightView,
    baseTheme
  ]
...
const gutterRightView = ViewPlugin.fromClass(class {
  ...
  constructor(readonly view: EditorView) {
    ...
    this.dom.className = "cm-gutters-right"
    this.dom.setAttribute("aria-hidden", "true")
    this.dom.style.minHeight = this.view.contentHeight + "px"
    this.gutters = view.state.facet(activeGuttersRight).map(conf => new SingleGutterView(view, conf))
    ...
    view.scrollDOM.insertBefore(this.dom, view.contentDOM.nextSibling)  // JAH: only change lol
  }
  ...
  updateGutters(update: ViewUpdate) {
    let prev = update.startState.facet(activeGuttersRight), cur = update.state.facet(activeGuttersRight)
...
const lineNumberGutter = activeGuttersRight.compute([lineNumberConfig], state => ({
...
export function lineNumbersRight(config: LineNumberConfig = {}): Extension {
  return [
